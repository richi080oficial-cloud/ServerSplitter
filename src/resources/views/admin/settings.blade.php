...
@section('content')
    ...
    @unless ($hasApiKey)
        <div class="alert alert-warning">
            ...
            <code>serversplitter apikey</code> para conectar WHMCS o Paymenter.
        </div>
    @endunless
        ...
        <div class="col-xs-12 col-md-7">
            <form action="{{ route('admin.serversplitter.settings.update') }}" method="POST">
                ...
                <div class="box box-primary">
                    <div class="box-header with-border"><h3 class="box-title">Configuracion general</h3></div>
                    <div class="box-body">
                        <div class="row">
        ...
        <div class="col-xs-12 col-md-5">
            <div class="box box-default">
                ...
                <div class="box-body">
                    ...
                    <p><strong>Servidores con limites propios:</strong> {{ $limits->count() }}</p>
                    <p><strong>Panel de cliente:</strong> <code>/serversplitter</code></p>
                    <p><strong>API de integracion:</strong> <code>/api/serversplitter/servers/&#123;server&#125;</code></p>
                </div>
                <div class="box-footer">
            ...
            <div class="box box-default">
                <div class="box-header with-border"><h3 class="box-title">Limites por servidor (WHMCS / Paymenter)</h3></div>
                <form action="{{ route('admin.serversplitter.limits.store') }}" method="POST">
                    @csrf
                    <div class="box-body">
                        <div class="form-group">
                            <label class="control-label" for="ss-limit-server">ID interno del servidor</label>
                            <input class="form-control" id="ss-limit-server" type="number" min="1" name="server_id" required>
                        </div>
                        <div class="row">
                            <div class="form-group col-xs-6">
                                <label class="control-label" for="ss-limit-splits">Max. divisiones</label>
                                <input class="form-control" id="ss-limit-splits" type="number" min="0" name="max_splits">
                            </div>
                            <div class="form-group col-xs-6">
                                <label class="control-label" for="ss-limit-memory">Max. memoria total</label>
                                <input class="form-control" id="ss-limit-memory" type="number" min="0" name="max_memory">
                            </div>
                            <div class="form-group col-xs-6">
                                <label class="control-label" for="ss-limit-disk">Max. disco total</label>
                                <input class="form-control" id="ss-limit-disk" type="number" min="0" name="max_disk">
                            </div>
                            <div class="form-group col-xs-6">
                                <label class="control-label" for="ss-limit-cpu">Max. CPU total</label>
                                <input class="form-control" id="ss-limit-cpu" type="number" min="0" name="max_cpu">
                            </div>
                        </div>
                    </div>
                    <div class="box-footer">
                        <button type="submit" class="btn btn-primary btn-sm pull-right">Guardar limites</button>
                    </div>
                </form>
                @if ($limits->count() > 0)
                    <div class="box-body table-responsive no-padding">
                        <table class="table table-hover">
                            ...
                            @foreach ($limits as $limit)
                                <tr>
                                    <td>{{ $limit->server?->name ?? ('#' . $limit->server_id) }}</td>
                                    <td>{{ $limit->max_splits ?: '-' }}</td>
                                    <td>{{ $limit->max_memory ?: '-' }}</td>
                                    <td>{{ $limit->max_disk ?: '-' }}</td>
                                    <td>{{ $limit->max_cpu ?: '-' }}</td>
                                    <td class="text-right">
                                        <form action="{{ route('admin.serversplitter.limits.destroy', $limit->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="row">
        <div class="col-xs-12 col-md-6">
            <div class="box box-default">
                <div class="box-header with-border"><h3 class="box-title">Reglas de eggs</h3></div>
                <form action="{{ route('admin.serversplitter.eggs.store') }}" method="POST">
                    ...
                    <div class="box-body">
                        ...
                        <p class="text-muted small">Deja un campo vacio para usar el valor global.</p>
                    </div>
                    <div class="box-footer">
        ...
        <div class="col-xs-12 col-md-6">
            <div class="box box-default">
                <div class="box-header with-border"><h3 class="box-title">Ultimas divisiones</h3></div>
                <div class="box-body table-responsive no-padding">
                    <table class="table table-hover">
                        <thead>
                        <tr>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center text-muted">Todavia no hay divisiones.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
